...
def draw_landmarks(img, pts, style='fancy', wfp=None, show_flag=False, **kwargs):
    ...
    for i in range(len(pts)):
        if dense_flag:
            ...
            center_coordinates = (int(pts[i][0][41]), int(pts[i][1][41])) #left eye
            cv2.circle(img, center_coordinates, 2, (255, 255, 255), -1)


            center_coordinates = (int(pts[i][0][30]), int(pts[i][1][30])) 
            cv2.circle(img, center_coordinates, 2, (255, 0, 0), -1)
            
            center_coordinates = (int(pts[i][0][47]), int(pts[i][1][47])) #right eye
            cv2.circle(img, center_coordinates, 2, (255, 255, 255), -1)

            center_coordinates = (int(pts[i][0][59]), int(pts[i][1][59])) #left corner of lips
            cv2.circle(img, center_coordinates, 2, (255, 255, 255), -1)

            center_coordinates = (int(pts[i][0][64]), int(pts[i][1][64])) #right corner of lips
            cv2.circle(img, center_coordinates, 2, (255, 255, 255), -1)
